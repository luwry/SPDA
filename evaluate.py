...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Change role lengths for the role length analysis.",
    )
...
def load_role_data(path):
    with open(path, 'r', encoding='utf-8')as f:
        ...
        schemas = []
        for i in schema:
            if len(i[0].split()) <= args.role_length and i not in schemas:
                schemas.append(i)
...
def evaluate_role(gold_roles, pred_roles, match_func, num_pred_roles):
...
if __name__ == '__main__':
    ...
    for event_type in EVENT_TYPE:
        ...
        if args.change_role_length:
            role2, event2 = load_role_data(pred_path)
            print("role_length:", args.role_length)
            print("change_role_length=True")
        else:
    ...
    print('Hard matching:')
    num = args.num_pred_roles
    print('num_pred_roles', num)
    pre_r, rec_r, f1_r = evaluate_role(gold_role, pred_role, match_fun, num)
